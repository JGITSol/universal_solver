...
agents = [
    ...
    Agent(
        name="Gemma",
        model="gemma3:1b",
        system_prompt="Solve math problems step by step with detailed reasoning.",
        temperature=0.2,
        max_tokens=512,
        provider="ollama",
    ),
    Agent(
        ...
        system_prompt="Be concise and accurate in your math solutions.",
        temperature=0.1,
        max_tokens=512,
        provider="ollama",
    ),
    ...
    Agent(
        name="LlamaNemotronUltra",
        model="nvidia/llama-3.1-nemotron-ultra-253b-v1:free",
        system_prompt="You are a SOTA LLM. Solve with detailed reasoning.",
        temperature=0.1,
        max_tokens=512,
        provider="openrouter",
    ),
    ...
    Agent(
        ...
        system_prompt="You are Google Gemini. Solve with step-by-step logic.",
        temperature=0.1,
        max_tokens=512,
        provider="gemini",
    ),
...
test_problems = [
    ...
    (
        "Calculus - Derivative",
        "Find the derivative of f(x) = x^4 * sin(x) with respect to x",
    ),
    (
        "Calculus - Integral",
        "Compute the definite integral of x^2 * e^x from x=0 to x=2",
    ),
    ("Optimization", "Find the maximum value of f(x) = -x^2 + 4x + 5"),
    ("Geometry", "What is the area of a triangle with sides 7, 8, and 9?"),
    (
        "Combinatorics",
        "How many ways can you arrange the letters in the word 'ALGEBRA'?",
    ),
]
...
def log_result_rich(solver_name, problem_type, problem, result):
    ...
    panel_title = (
        f"[bold blue]{solver_name}[/bold blue] | [magenta]{problem_type}[/magenta]"
    )
    if isinstance(result, dict):
        ...
        for sol in result.get("solutions", []):
            table.add_row(
                ...
                f"{sol['confidence']:.2f}",
                sol["explanation"][:80]
                + ("..." if len(sol["explanation"]) > 80 else ""),
            )
        theme_console.print(
            Panel(
                table,
                ...
                rows.append(
                    {
                        ...
                        "supporting_agents": ",".join(
                            getattr(result, "agents_in_agreement", [])
                        ),
                        "row_type": "agent",
                    }
                )
        ...
        for solver_name, solver in solvers:
            try:
                ...
                if solver_name == "RStarMathSolver":
                    ...
                    agent_solutions = [
                        solver.get_solution(agent, problem) for agent in agents
                    ]
