...
import pytest
from math_ensemble_adv_ms_hackaton import (
    Agent,
    MathProblemSolver,
    Solution,
    VotingResult,
...
def test_refine_solutions(mock_get, mock_ollama, sample_agents):
    ...
    discussion = (
        "Agent Expert: I think the answer is x=5\nAgent Creative: I agree, 5 is correct"
    )
...
def test_solve_high_initial_confidence(mock_get, mock_ollama, sample_agents):
    ...
    solver.vote_on_solutions = MagicMock(
    ...
    solver.get_solution = MagicMock(
        return_value=Solution(
            agent_name="Expert", answer="5", explanation="Explanation", confidence=0.9
        )
    )
...
def test_solve_with_discussion_rounds(mock_get, mock_ollama, sample_agents):
    ...
    # Mock methods
    solver.get_solution = MagicMock(
        side_effect=lambda agent, problem, prev=None: Solution(
            agent.name, "5", "Explanation", 0.4
        )
    )
    ...
    solver.refine_solutions = MagicMock(
        return_value=[
...
def test_solve_max_rounds_reached(mock_get, mock_ollama, sample_agents):
    ...
    solver.get_solution = MagicMock(
        side_effect=lambda agent, problem, prev=None: Solution(
            agent.name, "Different answer", "Explanation", 0.4
        )
    )

    # All votes have low confidence
    solver.vote_on_solutions = MagicMock(
        return_value=VotingResult("No consensus", 0.5, [])  # Always low confidence
    )

    # Mock discussion and refinement
    solver.facilitate_discussion = MagicMock(return_value="Mock discussion")
    solver.refine_solutions = MagicMock(
        side_effect=lambda problem, solutions, discussion: [
