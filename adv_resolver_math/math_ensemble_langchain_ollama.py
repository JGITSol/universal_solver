...
class MathEnsembleSolver:
    ...
    def _setup_environment(self):
        ...
        try:
            ...
            self.console.print(
                f"[red]Make sure Ollama is running at {self.ollama_base_url}[/red]"
            )
            self.console.print(f"[red]Error details: {e}[/red]")
            self.console.print(
                "[yellow]You can download Ollama from https://ollama.ai/[/yellow]"
            )
        ...
        with Progress(
            ...
            for model_name in self.models:
                task = progress.add_task(
                    f"[cyan]Initializing {model_name}...[/cyan]", total=1
                )

                try:
                    callback_handler = (
                        MathSolvingCallbackHandler(
                            console=self.console, model_name=model_name
                        )
                        if self.verbose
                        else None
                    )
                    ...
                    prompt = PromptTemplate(
                        template=MATH_PROMPT_TEMPLATE, input_variables=["problem"]
                    )
    ...
    def _load_cache(self):
        ...
        if self.use_cache and os.path.exists(self.cache_file):
            try:
                ...
                self.console.print(
                    f"[dim]Loaded {len(self.cache)} cached solutions[/dim]"
                )
            except Exception as e:
    ...
    def _score_solution(self, problem, solution):
        ...
        relevance_score = 0.2 * (
            len(problem_words.intersection(solution_words)) / max(1, len(problem_words))
        )
    ...
    def _solve_sequential(self, problem):
        ...
        for model_name, chain in self.model_chains.items():
            try:
                ...
                self.console.print(
                    f"[dim]{model_name} completed in {elapsed:.2f} seconds[/dim]"
                )
    ...
    def solve(self, problem):
        ...
        if self.mode == "sequential":
            ...
            raise ValueError(
                f"Unknown mode: {self.mode}. Use 'sequential' or 'parallel'."
            )
    ...
    def benchmark(self, dataset_name="gsm8k", split="test", num_samples=10):
        ...
        """
        self.console.print(
            f"[bold]Benchmarking on {dataset_name} dataset ({split} split)[/bold]"
        )
            ...
            for i, item in enumerate(sampled_items):
                ...
                for model, score in result.get("scores", {}).items():
                    ...
                    if (
                        ...
                        and (
                            extracted_answer in reference_answer
                            or reference_answer in extracted_answer
                        )
                    ):
    ...
    def _display_benchmark_summary(self, benchmark_result):
        ...
        # Create a table for the results
        table = Table(
            title=f"Benchmark Results ({benchmark_result['num_samples']} problems)"
        )
        table.add_column("Model", style="cyan")
    ...
    def _plot_benchmark_results(self, benchmark_result):
        ...
        try:
            ...
            # Average score chart
            avg_scores = [
                benchmark_result["avg_model_scores"][model] for model in models
            ]
            ...
            plot_path = os.path.join(
                self.cache_dir, f"benchmark_{benchmark_result['dataset']}.png"
            )
...
class MetaMathEnsemble:
        ...
        def classify_problem(problem):
            ...
            if "algebra" in detected_categories or "arithmetic" in detected_categories:
                return (
                    ...
                    "sequential",
                )  # Sequential for algebra/arithmetic
    ...
    def solve(self, problem):
        ...
        self.console.print(
            f"[dim]Problem classified as {category}, using {strategy} strategy[/dim]"
        )
