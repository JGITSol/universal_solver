...
def test_parallel_search_fact_sharing():
    ...
    search = EnhancedSKESTSearch(
        engine_factory=engine_factory_with_fact, num_threads=2, max_iterations=3
    )
    search.run_search()
...
def test_parallel_search_no_false_facts():
    ...
    search = EnhancedSKESTSearch(
        engine_factory=engine_factory_no_fact, num_threads=2, max_iterations=3
    )
    search.run_search()
    shared_facts = search.get_shared_knowledge()
